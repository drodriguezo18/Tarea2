...
export class CategoryService extends BaseService<ICategory> {
  ...
  getCategoriesByCategoryListId(categoryListId: number) {
    this.findAllWithParamsAndCustomSource(`category-list/${categoryListId}`, { page: this.search.page, size: this.search.size}).subscribe({
      next: (response: IResponse<ICategory[]>) => {
        this.search = {...this.search, ...response.meta};
        this.totalItems = Array.from({length: this.search.totalPages ? this.search.totalPages: 0}, (_, i) => i+1);
        this.categorySignal.set(response.data);
  ...
  addCategoryToCategoryList(categoryListId: number, category: ICategory) {
    this.addCustomSource(`../category-lists/${categoryListId}/categories`, category).subscribe({
      next: (response: IResponse<ICategory>) => {
  ...
  deleteCategoryFromCategoryList(categoryListId: number, categoryId: number) {
    this.delCustomSource(`../category-lists/${categoryListId}/categories/${categoryId}`).subscribe({
      next: (response: IResponse<ICategory>) => {
        this.alertService.displayAlert('success', response.message, 'center', 'top', ['success-snackbar']);
        this.getCategoriesByCategoryListId(categoryListId);
      },
      error: (err: any) => {
        this.alertService.displayAlert('error', 'An error occurred deleting the category', 'center', 'top', ['error-snackbar']);
        console.error('error', err);
